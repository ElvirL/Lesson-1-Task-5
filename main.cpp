...
Сектор выбирается так: с клавиатуры вводится офсет (смещение) относительно текущего сектора на барабане. Исходя из этого офсета 
вычисляется новый активный сектор, который и будет играть в этом ходе. Если выпавший сектор уже играл, выбирает следующий не 
игравший за ним. В начале всей игры стрелка установлена на первом секторе.
...
Игра продолжается до тех пор, пока или игрок, или зрители не наберут шесть баллов. После этого называется победитель и программа 
заканчивает работу.
*/
...
void choice_sector(bool game_sector[13], int& current_sector){
    cout << "Введите смещение сектора барабана: ";
    int shift;
    cin >> shift;
...
int main(){
    setlocale(LC_ALL, "Russian");
     ...
     while (gamer_score < 6 && viewers_score < 6){
        ...
        cout << "Счет:" << endl << "Знактоки - " << gamer_score << endl << "Телезрители - " << viewers_score << endl;
     }
